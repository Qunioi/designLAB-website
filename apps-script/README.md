...
- 密碼一律以加鹽迭代雜湊（SHA-256 × 2000 次）方式儲存，`USERS` 分頁的公開讀取
  ...
  升級為雜湊並清空明文欄位。
- 登入才會核發 Session Token（存在 Apps Script 的 `PropertiesService`，6 小時過期，
  每次使用會自動延長；到期時間存成 `expiresAt` 欄位自行判斷，因為 `PropertiesService`
  沒有原生 TTL），寫入、刪除、成員管理一律要求附上這個 Token，並在伺服器
...
- Session Token 原本存在 `CacheService`，但實測發現使用者常常在效期內就被強制
  登出——Google 官方文件承認 `CacheService` 在記憶體壓力大時可能提前清掉項目，
  不保證撐滿設定的效期。已改存 `PropertiesService`（真正持久化，不會被提前回收），
  效期改成自己存 `expiresAt` 時間戳記、讀取時手動判斷是否過期。`PropertiesService`
  總容量有上限（單一 Script Properties 最多 500 組、每組 9KB），對這個規模的團隊
  用量完全足夠；閒置帳號的過期 session 只會在下次被讀取到時才清掉，不會主動
  GC，但團隊人數少、單筆資料也很小，不會構成問題。
